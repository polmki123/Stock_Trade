...
if __name__ == '__main__': 
    ...
    printlog('total_cash :', total_cash)
    buy_amount = total_cash * 0.5  # 종목별 주문 금액 계산
    current_price, ask_price, bid_price = get_current_price("017670") 
    printlog('current_price :', current_price)
    # buy_qty = buy_amount // current_price*1.3 
    printlog('buy_qty :', "5")
    cpOrder.SetInputValue(0, "2")        # 2: 매수
    cpOrder.SetInputValue(1, acc)        # 계좌번호
    cpOrder.SetInputValue(2, "1" ) # 상품구분 - 주식 상품 중 첫번째
    cpOrder.SetInputValue(3, "017670")       # 종목코드
    cpOrder.SetInputValue(4, "15")    # 매수할 수량
    cpOrder.SetInputValue(5, current_price*1.3 )    # 매수할 금액 
